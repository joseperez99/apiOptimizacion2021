...
public class AlgorithmMapper {
    ...
    private Map<Integer,Long> getMapTime1(List<FrameDTO> frames)
    {
        ...
        for(FrameDTO frame : frames)
        {
            ...
            { timeMap.put(frame.getId(),
                        Duration.between(frame.getDepartureDateTime(),
                                frame.getArrivalDateTime()).toMinutes()); }
            else
            { timeMap.put(frame.getId(),
                        1440 - Duration.between(frame.getArrivalDateTime(),
                                frame.getDepartureDateTime()).toMinutes()); }
        }
        return timeMap;
    }
    ...
    private Map<Integer,Long> getMapTime(List<FrameDTO> frames, LocalTime previousArrivalTime)
    {
        ...
        {
            if (frame.getDepartureDateTime().isBefore(frame.getArrivalDateTime()))
            { tripDuration = Duration.between(frame.getDepartureDateTime(),
                        frame.getArrivalDateTime()).toMinutes(); }
            else
            { tripDuration = 1440 - Duration.between(frame.getArrivalDateTime(),
                        frame.getDepartureDateTime()).toMinutes(); }
            if (previousArrivalTime.isBefore(frame.getDepartureDateTime()))
            { waitDuration = Duration.between(previousArrivalTime,
                    frame.getDepartureDateTime()).toMinutes(); }
            else
            { waitDuration = 1440 - Duration.between(frame.getDepartureDateTime(),
                    previousArrivalTime).toMinutes(); }
